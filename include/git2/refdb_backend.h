...
typedef int (*git_refdb_lookup_cb)(
	const git_oid *oid,
	const char *name,
	void *payload);
...
struct git_refdb_backend {
	...
	/**
	 * Queries the refdb backend for a given reference.  If the reference
	 * exists, the callback will be called with the data.  A refdb
	 ...
	 * must provide this function.
	 */
	int (*delete)(struct git_refdb_backend *backend, const char *ref_name);
