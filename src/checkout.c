...
static int checkout_conflicts_load(checkout_data *data, git_iterator *workdir, git_vector *pathspec)
{
	...
	for (i = 0, cnt = git_index_reuc_entrycount(index); i < cnt; i++) {
		git_vector_insert(&data->reuc,
			(git_index_reuc_entry *)git_index_reuc_get_byindex(index, i));
	}

	for (i = 0, cnt = git_index_name_entrycount(index); i < cnt; i++) {
		git_vector_insert(&data->names,
			(git_index_name_entry *)git_index_name_get_byindex(index, i));
	}
...
static int checkout_extensions_update_index(checkout_data *data)
{
	...
	git_vector_foreach(&data->names, i, name_entry) {
		if ((error = git_index_name_add(data->index,
			name_entry->ancestor, name_entry->ours, name_entry->theirs)) < 0)
			goto done;
	}
	...
	const git_checkout_options *opts)
{
	...
	if ((error = checkout_extensions_update_index(&data)) < 0)
		goto cleanup;
...
cleanup:
	...
	git_vector_free(&data.reuc);
	git_vector_free(&data.names);
	git__free(actions);
	git__free(counts);
	checkout_data_clear(&data);
