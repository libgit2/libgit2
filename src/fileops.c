...
int git_futils_creat_withpath(const char *path, const mode_t dirmode, const mode_t mode)
{
	...
	if (fd < 0) {
		giterr_set_os("Failed to create file '%s'", path);
		return -1;
	}
...
int git_futils_creat_locked(const char *path, const mode_t mode)
{
	...
	if (fd < 0) {
		giterr_set_os("Failed to create locked file '%s'", path);
...
git_off_t git_futils_filesize(git_file fd)
{
	...
	if (p_fstat(fd, &sb)) {
		giterr_set_os("Failed to stat file descriptor");
		return -1;
	}
...
int git_futils_readbuffer_fd(git_buf *buf, git_file fd, size_t len)
{
	...
	if (read_size != (ssize_t)len) {
		giterr_set_os("Failed to read descriptor");
		git_buf_free(buf);
		return -1;
	}
	...
	git_buf *buf, const char *path, time_t *mtime, size_t *size, int *updated)
{
	...
	if (S_ISDIR(st.st_mode)) {
		giterr_set("requested file is a directory");
		return GIT_ENOTFOUND;
	}

	if (!git__is_sizet(st.st_size+1)) {
		giterr_set_os("Invalid regular file stat for '%s'", path);
		return -1;
	}
	...
	const git_buf *buf,	const char *path, int flags, mode_t mode)
{
	...
	if ((error = p_write(fd, git_buf_cstr(buf), git_buf_len(buf))) < 0) {
		giterr_set_os("Could not write to '%s'", path);
		(void)p_close(fd);
		return error;
	}
...
int git_futils_mv_withpath(const char *from, const char *to, const mode_t dirmode)
{
	...
	if (p_rename(from, to) < 0) {
		giterr_set_os("Failed to rename '%s' to '%s'", from, to);
		return -1;
	}
...
int git_futils_mmap_ro_file(git_map *out, const char *path)
{
	...
	if (!git__is_sizet(len)) {
		giterr_set_os("File `%s` too large to mmap", path);
		return -1;
	}
	...
	struct git_futils_mkdir_perfdata *perfdata)
{
	if ((S_ISREG(st->st_mode) && (flags & GIT_MKDIR_REMOVE_FILES)) ||
		...
		if (p_unlink(make_path) < 0) {
			giterr_set_os("Failed to remove %s '%s'",
		...
		if (p_mkdir(make_path, mode) < 0) {
			giterr_set_os("Failed to make directory '%s'", make_path);
			return GIT_EEXISTS;
		}
	}
	...
	struct git_futils_mkdir_options *opts)
{
	...
	if (make_path.size == 0) {
		giterr_set_os("Attempt to create empty path");
		goto done;
	}
...
retry_lstat:
		if (p_lstat(make_path.ptr, &st) < 0) {
			if (mkdir_attempted || errno != ENOENT) {
				giterr_set_os("Cannot access component in path '%s'", make_path.ptr);
				error = -1;
				goto done;
			}
			...
			if (p_mkdir(make_path.ptr, mode) < 0) {
				if (errno == EEXIST)
					goto retry_lstat;
				giterr_set_os("Failed to make directory '%s'", make_path.ptr);
				error = -1;
				goto done;
			}
		} else {
			...
			if ((flags & GIT_MKDIR_EXCL) != 0) {
				giterr_set("Failed to make directory '%s': directory exists", make_path.ptr);
				error = GIT_EEXISTS;
				goto done;
			}
		...
		if (((flags & GIT_MKDIR_CHMOD_PATH) != 0 ||
			...
			if ((error = p_chmod(make_path.ptr, mode)) < 0 &&
				lastch == '\0') {
				giterr_set_os("Failed to set permissions on '%s'",
					make_path.ptr);
				goto done;
			}
		}
	...
	if ((flags & GIT_MKDIR_VERIFY_DIR) != 0 &&
		...
		if (p_stat(make_path.ptr, &st) < 0 || !S_ISDIR(st.st_mode)) {
			giterr_set_os("Path is not a directory '%s'",
				make_path.ptr);
			error = GIT_ENOTFOUND;
		}
	}
...
{
	if (filemsg)
		giterr_set_os("Could not remove directory. File '%s' %s",
				   path, filemsg);
	else
		giterr_set_os("Could not remove directory '%s'", path);
...
static int cp_by_fd(int ifd, int ofd, bool close_fd_when_done)
{
	...
	if (len < 0) {
		giterr_set_os("Read error while copying file");
		error = (int)len;
	}

	if (error < 0)
		giterr_set_os("write error while copying file");
...
static int cp_link(const char *from, const char *to, size_t link_size)
{
	...
	if (read_len != (ssize_t)link_size) {
		giterr_set_os("Failed to read symlink data for '%s'", from);
		error = -1;
	}
	else {
		...
		if (p_symlink(link_data, to) < 0) {
			giterr_set_os("Could not symlink '%s' as '%s'",
				link_data, to);
			error = -1;
		}
	}
...
static int _cp_r_callback(void *ref, git_buf *from)
{
	...
	if (exists) {
		...
		if (p_unlink(info->to.ptr) < 0) {
			giterr_set_os("Cannot overwrite existing file '%s'",
				info->to.ptr);
			return GIT_EEXISTS;
		}
	}
	...
	if (info->flags & GIT_CPDIR_LINK_FILES) {
		if ((error = p_link(from->ptr, info->to.ptr)) < 0)
			giterr_set_os("failed to link '%s'", from->ptr);
