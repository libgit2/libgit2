...
{
	git_error *error = &GIT_TLSDATA->error_t;
	git_buf *buf = &GIT_TLSDATA->error_buf;

	error->message = buf->ptr;
	error->klass = error_class;

	GIT_TLSDATA->last_error = error;
}
...
{
	GIT_TLSDATA->last_error = &g_git_oom_error;
}
...
{
	if (GIT_TLSDATA->last_error != NULL) {
		set_error(0, NULL);
		GIT_TLSDATA->last_error = NULL;
	}
...
{
	return GIT_TLSDATA->last_error;
}

int git_error_state_capture(git_error_state *state, int error_code)
{
	git_error *error = GIT_TLSDATA->last_error;
	git_buf *error_buf = &GIT_TLSDATA->error_buf;
