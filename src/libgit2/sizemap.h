 ...
 */
#ifndef INCLUDE_sizemap_h__
#define INCLUDE_sizemap_h__
...
typedef struct kh_size_s git_sizemap;
...
#define git_sizemap_foreach(h, kvar, vvar, code) { size_t __i = 0;		\
	while (git_sizemap_iterate((void **) &(vvar), h, &__i, &(kvar)) == 0) {	\
		code;								\
	} }

#define git_sizemap_foreach_value(h, vvar, code) { size_t __i = 0;		\
	while (git_sizemap_iterate((void **) &(vvar), h, &__i, NULL) == 0) {	\
		code;								\
	} }
