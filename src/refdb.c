...
static int refdb_populate_ref(
	const git_oid *oid, const char *symbolic, void *payload)
{
	git_reference *ref = payload;
	...
	if (oid) {
		...
		ref->target.symbolic = git__strdup(symbolic);
...
int git_refdb_lookup(git_refdb *db, git_reference *ref)
{
	assert(db && db->backend && ref);

	return db->backend->lookup(db->backend, ref->name, refdb_populate_ref, ref);
}
...
int git_refdb_write(git_refdb *db, git_reference *ref)
{
	assert(db && db->backend);
...
int git_refdb_compress(struct git_refdb *db)
{
	assert(db && db->backend);
