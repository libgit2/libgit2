	...
	void *payload)
{
	...
	result = reference_read(&ref_file, NULL, backend->path, ref_name, NULL);
	...
	if (git__prefixcmp((const char *)(ref_file.ptr), GIT_SYMREF) == 0) {
		...
		callback(NULL, target, payload);
	} else {
		if (loose_parse_oid(&oid, &ref_file) < 0)
			return -1;
		
		callback(&oid, NULL, payload);
	}
	...
	void *payload)
{
	...
	if (callback(&entry->oid, NULL, payload) != 0)
		return GIT_EUSER;
	...
	void *payload)
{
	...
	if ((result = loose_lookup(backend, ref_name, callback, payload)) == 0)
		return 0;
	...
	if (result == GIT_ENOTFOUND) {
		giterr_clear();
		result = packed_lookup(backend, ref_name, callback, payload);
	}
	...
	const char *ref_name)
{
	...
	if (git_buf_joinpath(&loose_path, repo->path_repository, ref_name) < 0)
		return -1;
	...
	if ((pack_error = packed_map_entry(&pack_ref, &pack_ref_pos, backend, ref_name)) == 0) {
