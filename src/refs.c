...
static int reference_lookup(git_reference *ref)
{
	...
	result = git_refdb_lookup(ref->db, ref);
...
{
	int result = git_refdb_delete(ref->db, ref->name);
	git_reference_free(ref);
	return result;
}
	...
	int max_nesting)
{
	...
	*ref_out = NULL;
	...
	if ((result = git_reference__normalize_name_lax(scan->name,
		GIT_REFNAME_MAX, name)) < 0) {
		git_reference_free(scan);
		return result;
	}

	git_repository_refdb__weakptr(&scan->db, repo);
	scan->type = GIT_REF_SYMBOLIC;

	for (nesting = max_nesting;
		 nesting >= 0 && scan->type == GIT_REF_SYMBOLIC;
		 nesting--)
	{
	...
	if (scan->type != GIT_REF_OID && max_nesting != 0) {
		giterr_set(GITERR_REFERENCE,
...
int git_reference_set_target(git_reference *ref, const git_oid *id)
{
	...
	/* Write back to disk */
	return git_refdb_write(ref->db, ref);
}
...
int git_reference_rename(git_reference *ref, const char *new_name, int force)
{
	 ...
	 */
	if (git_refdb_delete(ref->db, ref->name) < 0)
		goto cleanup;
...
int git_reference_compress_all(git_repository *repo)
{
	git_refdb *refdb;
	git_repository_refdb__weakptr(&refdb, repo);

	return git_refdb_compress(refdb);
}
