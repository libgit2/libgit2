...
static int retrieve_tag_reference(git_reference **tag_reference_out, git_path *ref_name_out, git_repository *repo, const char *tag_name)
{
	...
	*tag_reference_out = NULL;
	...
	error = git_path_join(ref_name_out, GIT_REFS_TAGS_DIR, tag_name);
	if (error < GIT_SUCCESS)
		return git__rethrow(error, "Failed to retrieve tag reference");

	error = git_reference_lookup(&tag_ref, repo, ref_name_out->data);
	if (error < GIT_SUCCESS)
		return git__rethrow(error, "Failed to retrieve tag reference");
		...
		int create_tag_annotation)
{
	...
	git_path ref_name = GIT_PATH_INIT;
	...
	if (!should_update_ref)
		error = git_reference_create_oid(&new_ref, repo, ref_name.data, oid, 0);
	else
