...
GIT_INLINE(void *) git__malloc(size_t len)
{
	void *ptr = malloc(len);
	if (!ptr)
		git__catcherror(GIT_ENOMEM, "Out of memory. Failed to allocate %d bytes.", (int)len);
	return ptr;
}
...
GIT_INLINE(void *) git__calloc(size_t nelem, size_t elsize)
{
	...
	if (!ptr)
		git__catcherror(GIT_ENOMEM, "Out of memory. Failed to allocate %d bytes.", (int)elsize);
	return ptr;
}
...
GIT_INLINE(char *) git__strdup(const char *str)
{
	char *ptr = strdup(str);
	if (!ptr)
		git__catcherror(GIT_ENOMEM, "Out of memory. Failed to duplicate string");
	return ptr;
}
...
GIT_INLINE(void *) git__realloc(void *ptr, size_t size)
{
	...
	if (!new_ptr)
		git__catcherror(GIT_ENOMEM, "Out of memory. Failed to allocate %d bytes.", (int)size);
	return new_ptr;
}
