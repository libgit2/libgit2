...
#define TEST_OID_PASS(string, header) {                           \
      ...
      cl_git_pass(git_oid__parse(&oid, &ptr, ptr+len, header));   \
      cl_assert(ptr == ptr_original + len);                       \
   }
#define TEST_OID_FAIL(string, header) {                           \
      const char *ptr = string;                                   \
      size_t len = strlen(ptr);                                   \
      cl_git_fail(git_oid__parse(&oid, &ptr, ptr+len, header));   \
   }
...
#define TEST_SIGNATURE_PASS(_string, _header, _name, _email, _time, _offset) { \
      ...
      cl_git_pass(git_signature__parse(&person, &ptr, ptr + len, _header, '\n')); \
      cl_assert(strcmp(_name, person.name) == 0);                       \
      cl_assert(strcmp(_email, person.email) == 0);                     \
      cl_assert(_time == person.when.time);                             \
      cl_assert(_offset == person.when.offset);                         \
...
#define TEST_SIGNATURE_FAIL(_string, _header) {                         \
      ...
      size_t len = strlen(_string);                                     \
      git_signature person = {NULL, NULL, {0, 0}};                      \
      cl_git_fail(git_signature__parse(&person, &ptr, ptr + len, _header, '\n')); \
      git__free(person.name); git__free(person.email);                  \
   }
