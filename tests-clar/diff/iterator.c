	...
	const char *an_ignored_name)
{
	...
	cl_git_pass(git_iterator_for_workdir(&i, repo, GIT_ITERATOR_DONT_AUTOEXPAND, start, end));
	cl_git_pass(git_iterator_current(&entry, i));
...
void test_diff_iterator__workdir_builtin_ignores(void)
{
	...
	cl_git_pass(
		git_iterator_for_workdir(&i, repo, GIT_ITERATOR_DONT_AUTOEXPAND, "dir", "sub/sub/file"));
	cl_git_pass(git_iterator_current(&entry, i));
