...
static void make_dummy_hook(const char *hook_name)
{
	...
	cl_git_pass(git_hook_dir(&hook_dir, repo));

	cl_must_pass(p_mkdir(hook_dir, 0777));

	git_buf_joinpath(&hook_path, hook_dir, hook_name);
...
void test_hook_call__pre_rebase_hook(void)
{
	...
	cl_git_pass(git_hook_register_callback(repo, hook_exec__pre_rebase, NULL, NULL));
	...
	cl_git_fail_with(git_rebase_init(&rebase, repo, branch_head, upstream_head, NULL, NULL), -1);
