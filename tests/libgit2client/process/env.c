...
static void run_env(const char **env, size_t env_len, bool exclude_env)
{
	const char *args[] = { env_cmd.ptr };
	...
	cl_git_pass(git_process_new(&process, args, 1, env, env_len, &opts));
