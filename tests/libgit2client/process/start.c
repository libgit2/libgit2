...
{
#ifdef GIT_WIN32
	const char *args[] = { "C:\\Windows\\System32\\cmd.exe", "/c", "exit", "1" };
#else
	const char *args[] = { "/usr/bin/false" };
#endif
	...
	cl_git_pass(git_process_new(&process, args, ARRAY_SIZE(args), NULL, 0, &opts));
...
{
#ifdef GIT_WIN32
	const char *args[] = { "C:\\a\\b\\z\\y\\not_found" };
#else
	const char *args[] = { "/a/b/z/y/not_found" };
#endif
...
static void read_all(git_buf *out, git_process *process)
{
	char buf[32];
	size_t buf_len = 32;
	ssize_t ret;
	...
	while ((ret = git_process_read(process, buf, buf_len)) > 0)
		cl_git_pass(git_buf_put(out, buf, ret));
...
{
#ifdef GIT_WIN32
	const char *args[] = { "C:\\Windows\\System32\\cmd.exe", "/c", cat_cmd.ptr };
#else
...
{
#ifndef GIT_WIN32
	const char *args[] = { helloworld_cmd.ptr };
...
{
#ifdef GIT_WIN32
	const char *args[] = { "C:\\Windows\\System32\\cmd.exe", "/c", pwd_cmd.ptr };
	char *startwd = "C:\\";
#else
