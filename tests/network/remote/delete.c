...
void test_network_remote_delete__cannot_delete_an_in_memory_remote(void)
{
	git_remote *inmemory;

	cl_git_pass(git_remote_create_inmemory(&inmemory, _repo, NULL, "git://github.com/libgit2/libgit2"));

	cl_git_fail(git_remote_delete(inmemory));

	git_remote_free(inmemory);
}
...
void test_network_remote_delete__deleting_a_remote_removes_the_branch_remote_configuration_settings(void)
{
	...
	assert_config_entry_existence(_repo, "branch.merge.remote", false);
}
